"""Upload Gemma 4B outlines to HuggingFace with rate limit handling."""
import os
import time
...
# Config
HF_REPO_ID = "yulia-volkova/parascopes-outlines-gemma4b"
RESULTS_DIR = Path(__file__).parent / "results"
SOURCE_DIR = RESULTS_DIR / "fineweb-gemma4b"
CACHE_DIR = RESULTS_DIR / ".hf_cache"  # Local cache directory
...
os.environ["HF_HOME"] = str(CACHE_DIR)  # Set HF cache directory
...
def get_uploaded_files():
    ...
    try:
        ...
        return set(f for f in files if f.endswith('.parquet'))
    ...
    for attempt in range(MAX_RETRIES):
        try:
            api.upload_file(
                ...
                path_in_repo=f"v0.0/data/outlines_{chunk_id:03d}.parquet",
                repo_id=HF_REPO_ID,
                repo_type="dataset"
            )
...
def main():
    """Upload all generated outlines to HuggingFace with rate limit handling."""
    ...
    local_files = sorted((SOURCE_DIR / "v0.0").glob("outlines_*.parquet"))
    ...
    for local_file in local_files:
        ...
        remote_name = f"v0.0/data/outlines_{chunk_id:03d}.parquet"
